...
// ============================================
router.get('/', ratelimit_1.cleanupRateLimiter, verifyCronSecret, async (req, res) => {
    const startTime = Date.now();
    try {
        const now = new Date();
        // Find and mark time-expired pastes
        const timeExpired = await prisma_1.prisma.paste.updateMany({
            where: {
                isExpired: false,
                expiresAt: {
                    lte: now,
        ...
        const viewExpiredResult = await prisma_1.prisma.$executeRaw `
      UPDATE "Paste" 
      SET "isExpired" = true 
    ...
    `;
        // Get statistics
        const totalExpired = await prisma_1.prisma.paste.count({
            where: { isExpired: true },
        });
        const totalActive = await prisma_1.prisma.paste.count({
            where: { isExpired: false },
        });
        const oldestActive = await prisma_1.prisma.paste.findFirst({
            where: { isExpired: false },
...
// ============================================
router.post('/purge', ratelimit_1.cleanupRateLimiter, verifyCronSecret, async (req, res) => {
    const startTime = Date.now();
    try {
        // Optional: Only purge pastes expired for more than X days
        const retentionDays = parseInt(req.query.retentionDays) || 7;
        const cutoffDate = new Date();
        cutoffDate.setDate(cutoffDate.getDate() - retentionDays);
        // Count before deletion
        const toDelete = await prisma_1.prisma.paste.count({
            where: {
                isExpired: true,
                createdAt: {
                    lte: cutoffDate,
        ...
        const deleted = await prisma_1.prisma.paste.deleteMany({
            where: {
                isExpired: true,
                createdAt: {
                    lte: cutoffDate,
...
router.get('/stats', verifyCronSecret, async (req, res) => {
    try {
        const [totalPastes, activePastes, expiredPastes, totalViews, pastesWithExpiry, pastesWithMaxViews,] = await Promise.all([
            prisma_1.prisma.paste.count(),
            prisma_1.prisma.paste.count({ where: { isExpired: false } }),
            prisma_1.prisma.paste.count({ where: { isExpired: true } }),
            prisma_1.prisma.paste.aggregate({ _sum: { viewCount: true } }),
            prisma_1.prisma.paste.count({ where: { expiresAt: { not: null } } }),
            prisma_1.prisma.paste.count({ where: { maxViews: { not: null } } }),
        ]);
        // Get pastes by language
        const byLanguage = await prisma_1.prisma.paste.groupBy({
            by: ['language'],
            _count: true,
